...
export const invoiceSchema = Joi.object({
  noInvoice: Joi.string()
    .pattern(/^INV-gen-\d{8}-\d{3}$/)
    .required()
    .messages({
      "string.base": "Invalid type: expected a string",
      "string.pattern.base":
        "Invalid No Invoice format. Example: INV-gen-YYYYMMDD-XXX",
  ...
  phoneNumber: Joi.string()
    .pattern(/^\+\d{1,3}\s[1-9]\d{9,14}$/)
    .required()
    .messages({
      "string.base": "Invalid type: expected a string",
      "string.pattern.base":
        "Invalid Phone Number format. Example: +62 8123456XXXX",
